...
- **Use `localStorage` for console simulation state**: The console browser tab is the sole authoritative simulator for a room and simulation state only needs to survive same-device tab refreshes. Game simulation states (`grid-dungeon`, `flappy-royale`, `liars-dice`, `othello`, `uno`) MUST be saved locally via `ctx.storage.saveRoomState(state)` rather than sending RPC calls to the Durable Object.
